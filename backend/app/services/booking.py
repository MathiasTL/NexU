...
def _to_response(
    ...
    if user_repo is not None:
        tenant = user_repo.get_by_id(b.tenant_id)
...
class BookingService:
    ...
    def get_by_tenant(self, tenant_id: int) -> list[BookingResponse]:
        return [_to_response(b, self._properties, self._users) for b in self._bookings.get_by_tenant_id(tenant_id)]

    def get_by_host(self, host_id: int) -> list[BookingResponse]:
        return [_to_response(b, self._properties, self._users) for b in self._bookings.get_by_host_id(host_id)]
    ...
    def create(self, req: CreateBookingRequest) -> BookingResponse:
        ...
        return _to_response(booking, self._properties, self._users)
