      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
        <div className="mb-6 flex items-center gap-3">
          <button
            ...
            className="rounded-xl border border-gray-200 p-2 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
          >
        ...
        {results.length === 0 ? (
          <div className="rounded-2xl border border-gray-100 bg-white py-16 text-center dark:border-gray-700 dark:bg-gray-800">
            <p className="text-gray-500 dark:text-gray-400">No encontramos espacios con esos criterios.</p>
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      ...
      <div className="mb-8 text-center">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Encuentra tu espacio ideal</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Responde {STEPS.length} preguntas rápidas y te mostramos las mejores opciones</p>
      </div>

      {/* Progress */}
      <div className="mb-8 flex gap-1.5">
        {STEPS.map((_, i) => (
          <div key={i} className={cn('h-1.5 flex-1 rounded-full transition-colors', i <= step ? 'bg-primary' : 'bg-gray-200 dark:bg-gray-700')} />
        ))}
      </div>

      {/* Steps */}
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Paso {step + 1} de {STEPS.length}</p>
        <h2 className="mb-6 text-lg font-bold text-gray-900 dark:text-white">{STEPS[step]}</h2>

        {/* Step 0: Universidad */}
        {step === 0 && (
          <div>
            <p className="mb-3 text-sm text-gray-600 dark:text-gray-400">¿Cerca de qué universidad buscas alojamiento?</p>
        ...
        {step === 1 && (
          <div>
            <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">¿Cuánto puedes pagar al mes?</p>
            <div className="mb-2 flex items-center justify-between">
              <span className="text-sm text-gray-500 dark:text-gray-400">Presupuesto máximo</span>
          ...
          <div className="flex flex-col gap-3">
            {ROOM_TYPES.map(rt => (
              <button key={rt.value} type="button" onClick={() => setRoomType(rt.value)}
                ...
                <div>
                  <p className={cn('font-semibold', roomType === rt.value ? 'text-primary-700 dark:text-primary' : 'text-gray-900 dark:text-white')}>
        ...
        {step === 3 && (
          <div className="flex flex-col gap-5">
            <div>
              <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">¿Cuándo te acuestas?</p>
                ...
                {[{ label: '🌅 Antes de las 11pm', value: 'early' as const }, { label: '🌙 Después de medianoche', value: 'night' as const }].map(opt => (
                  <button key={opt.value} type="button" onClick={() => setSleepSched(v => v === opt.value ? '' : opt.value)}
                    className={cn('flex-1 rounded-xl border px-3 py-2 text-sm font-medium transition-colors',
                      sleepSched === opt.value
                        ? 'border-primary bg-primary text-white'
                        : 'border-gray-200 text-gray-600 hover:border-primary-300 dark:border-gray-600 dark:text-gray-300 dark:hover:border-primary/50')}>
                    {opt.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">¿Cómo estudias en casa?</p>
              <div className="flex gap-2">
            ...
            <div>
              <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Ambiente que prefieres</p>
              <div className="flex gap-2">
