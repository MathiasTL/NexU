...
export const UniversityCombobox = ({
  ...
  const handleInput = (v: string) => { setInputValue(v); onChange(v); setOpen(true) }
  const handleSelect = (u: string) => { setInputValue(u); onChange(u); setOpen(false) }
  const handleClear  = (e: React.MouseEvent) => {
    e.stopPropagation(); setInputValue(''); onChange(''); setOpen(false)
              ...
              <li key={u}>
                <button
                  ...
                  className={cn(
                    ...
                    inputValue === u ? 'bg-primary-50 text-primary-600' : 'text-gray-700 hover:bg-gray-50',
                  )}
                >
                  <span className={cn(
                    'flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold',
                    inputValue === u ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-500',
                  )}>
                    {u[0]}
                  </span>
                  <span className="font-medium">{u}</span>
                  {inputValue === u && <span className="ml-auto text-xs text-primary-400">✓</span>}
                </button>
              </li>
            ))}
          </ul>
  ...
  if (embedded) {
    return (
      <div ref={wrapperRef} className={cn('relative flex items-center gap-2', className)}>
        ...
        />
        {inputValue
          ? <button type="button" onMouseDown={handleClear} className="shrink-0 text-gray-400 hover:text-gray-600"><X className="h-3.5 w-3.5" /></button>
  ...
  return (
    <div ref={wrapperRef} className={cn('relative', className)}>
      ...
      >
        <GraduationCap className={cn('h-4 w-4 shrink-0 transition-colors', open ? 'text-primary' : 'text-gray-400')} />
        <input
          ref={inputRef}
          type="text"
          value={inputValue}
